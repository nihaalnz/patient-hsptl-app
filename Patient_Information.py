...
def manage():
    ...
    l_head = Label(update, text='Administration Panel', font=Font(size='20'))
    ...
    def updates():
        ...
        if values == "":
            messagebox.showinfo('No value!', 'Please enter an ID in the box')
    ...
    btn_sp = ttk.Button(update, text='Search Patients', command=sp_patient)
    btn_all = ttk.Button(update, text='View all Patients',
                         command=all_patients)
    btn_ext = ttk.Button(update, text='Exit', command=update.destroy)
...
def imgpath():
    ...
    path = filedialog.askopenfilename(
        initialdir='/Donwloads', title='Select Photo', filetypes=(('JPEG files', '*.jpg'), ('PNG files', '*.png')))
